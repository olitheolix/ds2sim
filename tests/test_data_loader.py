...
class TestDataset:
    ...
    def test_limit_dataset_size(self):
        ...
        # which means a total of 3 features in the loader.
        ds = loader.DataSet(train=1, N=1)
        assert ds.lenOfEpoch('train') == 3

    def test_basic_err(self):
        with pytest.raises(AssertionError):
            loader.DataSet(train=-1)
        with pytest.raises(AssertionError):
            loader.DataSet(train=1.1)
    ...
    def test_basic_with_param(self):
        ...
        for labels, expected in sorted(testcases.items()):
            ds = loader.DataSet(train=1, labels=labels)
        ...
        with pytest.raises(AssertionError):
            loader.DataSet(train=1, labels={'foo'})
